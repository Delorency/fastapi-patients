...
@inject
def create_patient(schema = Depends(PatientCreateRequest), service = Depends(Provide[Container.patient_service])) -> PatientSchema:
    return service.create(schema)


@router.put("/{id}", summary="Full patient update", status_code=200)
@inject
def full_patient_update(id:int, schema = Depends(PatientUpdateRequest), service = Depends(Provide[Container.patient_service])) -> PatientSchema:
    return service.full_update(id, schema)


@router.patch("/{id}", summary="Partial patient update", status_code=200)
@inject
def partial_patient_update(id:int, schema = Depends(PatientUpdateRequest), service = Depends(Provide[Container.patient_service])) -> PatientSchema:
    return service.partial_update(id, schema)


@router.post("/{id}/doctor/add", summary="Add doctor to patient", status_code=200)
@inject
def add_doctor_to_patient(id:int, schema = Depends(PatientChangeDoctorsRequest), service = Depends(Provide[Container.patient_service])) -> None:
...
@inject
def remove_doctor_from_patient(id:int, schema = Depends(PatientChangeDoctorsRequest), service = Depends(Provide[Container.patient_service])) -> None:
